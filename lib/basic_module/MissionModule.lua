...
MissionModule.show_mission_objective = function(mission_name, is_show)
    if type(mission_name) ~= 'string' then
        LoggerModule.error('MissionModule.show_mission_objective', 'mission_name must be a string')
        return
    end

    if is_show then
        ExecuteAction('SHOW_MISSION_OBJECTIVE', mission_name) -- (403)
    else
        ExecuteAction('HIDE_MISSION_OBJECTIVE', mission_name) -- (404)
    end
end
...
MissionModule.link_mission_objective = function(mission_name, is_enable, mission_object_category, link_to_unit_table_or_name, link_to_waypoint_name)
    if type(mission_name) ~= 'string' then
        LoggerModule.error('MissionModule.link_mission_objective', 'mission_name must be a string')
        return
    end
    ...
    elseif link_to_unit_table_or_name ~= nil then
        if mission_object_category == nil then
            ExecuteAction('LINK_MISSION_OBJECTIVE_TO_OBJECT', mission_name, link_to_unit_table_or_name, is_enable_value) -- (405)
        else
            ExecuteAction('LINK_MISSION_OBJECTIVE_TO_OBJECT_WITH_CATEGORY', mission_name, link_to_unit_table_or_name, is_enable_value, mission_object_category) -- (553)
        end
    else
        if mission_object_category == nil then
            ExecuteAction('LINK_MISSION_OBJECTIVE_TO_WAYPOINT', mission_name, link_to_waypoint_name, is_enable_value) -- (406)
        else
            ExecuteAction('LINK_MISSION_OBJECTIVE_TO_WAYPOINT_WITH_CATEGORY', mission_name, link_to_waypoint_name, is_enable_value, mission_object_category) -- (552)
        end
    end
end
...
MissionModule.set_mission_objective_status = function(mission_name, status)
    if type(mission_name) ~= 'string' then
        LoggerModule.error('MissionModule.set_mission_objective_status', 'mission_name must be a string')
        return
    end
    ...
    ExecuteAction('SET_MISSION_OBJECTIVE_STATUS', mission_name, status) -- (492)
end
