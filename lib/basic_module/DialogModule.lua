...
DialogModule.show_choice_dialog = function(dialog_id, title, button_contents, player_name_list)
    if type(player_name_list) ~= "table" and player_name_list ~= nil and type(player_name_list) ~= 'string' then
        LoggerModule.error('ChoiceDialogModule.show_choice_dialog', 'player_name_list type error:' .. type(player_name_list))
        return
    end
    if type(dialog_id) ~= "number" then
        LoggerModule.error('ChoiceDialogModule.show_choice_dialog', 'dialog_id type error:' .. type(dialog_id))
        return
    end
    if type(title) ~= "string" then
        LoggerModule.error('ChoiceDialogModule.show_choice_dialog', 'title type error:' .. type(title))
        return
    end
    if type(button_contents) ~= "table" then
        LoggerModule.error('ChoiceDialogModule.show_choice_dialog', 'button_contents type error:' .. type(button_contents))
        return
    end
    ...
    elseif type(player_name_list) == 'table' then
        for i = 1, getn(player_name_list) do
            if type(player_name_list[i]) ~= "string" then
                LoggerModule.warn('ChoiceDialogModule.show_choice_dialog', 'player_name_list element type error:' .. type(player_name_list[i]))
            else
