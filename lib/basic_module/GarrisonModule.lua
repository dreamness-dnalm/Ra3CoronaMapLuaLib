...
--- 使单位离开建筑 (96)
--- @param child_unit_name string
GarrisonModule.leave = function(child_unit_name)
    LoggerModule.debug("GarrisonModule.leave", "child_unit_name: " .. tostring(child_unit_name))
    if type(child_unit_name) ~= "string" then
        LoggerModule.error("GarrisonModule.leave", "child_unit_name must be a string")
        return
    end
    ExecuteAction("NAMED_EXIT_BUILDING", child_unit_name)
